...
class GetAllSitesOfSuperCompanyTest extends Simulation {
  ...
  var token="";
	 ...
	 */
	object getAllSites{
		val getAllSitesRequest=
				feed(loginData)
				.exec(session => session.set("token",token))
				.exec(http("Get list of Sites of a super companies")
						.get("/companies/${superCompanyId}/sites")
						.headers(authToken)  
	...
	val httpConf =http
			...
			val getSitesList = scenario("get all sites of super companies").exec(getAllSites.getAllSitesRequest)




			setUp(loginBackuser.inject(nothingFor(2 seconds),atOnceUsers(1)),
					getSitesList.inject(nothingFor(10 seconds),splitUsers(20) into atOnceUsers(1) separatedBy(10 seconds)
							)).protocols(httpConf)
