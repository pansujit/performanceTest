...
class GetCompanies extends Simulation {
  ...
  var token="";
	 ...
	 */
	object getcompanies{
		val getcompaniesRequest=
				feed(loginData)
				.exec(session => session.set("token",token))
				.exec(http("Get list of companies of a super companies")
						.get("/companies/${superCompanyId}/sub-companies")
						.headers(authToken)  
	...
	val httpConf =http
			...
			val getCompanyDetails = scenario("GetCompaniesTest").exec(getcompanies.getcompaniesRequest)


			setUp(loginBackuser.inject(nothingFor(2 seconds),atOnceUsers(1)),
					getCompanyDetails.inject(nothingFor(10 seconds),splitUsers(20) into atOnceUsers(1) separatedBy(10 seconds)
							)).protocols(httpConf)
