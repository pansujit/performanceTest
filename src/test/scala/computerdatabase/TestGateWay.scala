...
class TestGateWay extends Simulation {
	...
	object searchFilterted{
		val searchFilteredRequest=feed(searchAddressData)  
				...
				//.exec(session => session.set("startDate", SetTime.startTime(15)))
				//.exec(session => session.set("endDate", SetTime.endDate(75)))
	...
	val login=feed(loginData)
				...
				.exec(http("login_request")
						...
						.body(StringBody("""{ "login": "super.admin@ekino.com",
								"password":"1Aaaaaaa" }""")).asJSON
						.check(header("x-auth-token").saveAs("auth_token"))  
						.check(jsonPath("$.id").saveAs("id")))
				.pause(2)
	}

// This method get the single member details
	object getMember{
		val getmember=exec(http("getMember")
				.get("/members/${id}")
				.headers(authToken)
				.check(status.is(200))
				.check(jsonPath("$.login").is("${username}"))
				)
				pause(10)
	}
/**
 ...
 */
	...
	val httpConf = http
      ...
      .disableCaching
			...
			val authToken= Map("X-AUTH-TOKEN" -> "${auth_token}") 
			...
			val scn = scenario("search filetered by user").exec(authetication.login,searchFilterted.searchFilteredRequest)
