...
class GetSuperCompanyTest extends Simulation {
	 ...
	 */
	object getSuperCompany{
		val getSuperCompanyRequest=
				feed(loginData)
				.exec(session => session.set("token",token))
				.exec(http("Get single super companies")
						.get("/companies/${superCompanyId}")
						.headers(authToken)  
	...
	val httpConf =http
			...
			val getSuperCompanyDetails = scenario("GetSuperCompanyTest").exec(getSuperCompany.getSuperCompanyRequest)




			setUp(loginBackuser.inject(nothingFor(2 seconds),atOnceUsers(1)),
					getSuperCompanyDetails.inject(nothingFor(5 seconds),splitUsers(20) into atOnceUsers(1) separatedBy(10 seconds)
							)).protocols(httpConf)
