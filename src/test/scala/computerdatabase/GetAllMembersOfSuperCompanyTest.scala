...
class GetAllMembersOfSuperCompanyTest extends Simulation {
  var token="";
	 ...
	 */
	object getALlMemberOfSC{
		val getALlMemberOfSCRequest=
				feed(loginData)
				.exec(session => session.set("token",token))
				.exec(http("Get list of members of a super companies")
						.get("/companies/${superCompanyId}/members")
						.headers(authToken)  
	...
	val httpConf =http
			...
			val getAllMemberList= scenario("Get all members of super companies").exec(getALlMemberOfSC.getALlMemberOfSCRequest)


			setUp(loginBackuser.inject(nothingFor(2 seconds),atOnceUsers(1)),
					getAllMemberList.inject(nothingFor(10 seconds),splitUsers(20) into atOnceUsers(1) separatedBy(10 seconds)
							)).protocols(httpConf)
